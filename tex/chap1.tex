...
\par{
W literaturze [3] można spotkać następującą, nieco bardziej ścisłą definicję:g
Model \textit{to operator przekształcający zadane zmienne wejściowe X(t) w zmieenne wyjściowe Y(t) czyli 
\begin{center}
$Y(t) = H_{t} \times X(t).$
\end{center}
}
...
\subsubsection{Zasady tworzenia modeli}
\par{
Literatura [2] podaje następujące aspekty, które powinny przykuć uwagę projektanta podczas modelowania jakichkolwiek zjawisk.
\begin{itemize}
\item \textbf{poziom uproszczenia} - model z samej swojej natury musi być pewnym uproszczonym wycinkiem rzeczywistości ...
\item \textbf{potrzeby eksperymentu}
\item \textbf{poprawność}
\item \textbf{przydatność}
\item \textbf{wiarygodność}
\item \textbf{cel}
\end{itemize}
}
