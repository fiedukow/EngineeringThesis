...
\par{
Przy planowaniu badań zauważono, że jeśli postawiona hipoteza była by prawdziwa, to kluczowe z punktu widzenia badanego czasu pojedynczej pętli była by ilość odczytów jakie czyniły czujniki w czasie takiego obiegu. Z tego względu zdecydowano się w ramach planowania właściwego eksperymentu na zbadanie zależności między ilością obiektów w symulacji a czasem obiegu pojedynczej symulacji. Efekt tej pracy przedstawia \textit{Tablica 4.2} oraz \textit{Wykres 1}.
}
\par{
\begin{table}[t]
...
\par{
Na \textit{wykresie 2} wyraźnie widać, że zarówno przypadku zapisu do bazy danych jak i gdy zapis ten zostanie wyłączony czas trwania pojedynczego obiegu pętli zależy liniowo od ilości obiektów w symulacji.
}
\par{
